  ...
  const menuSections = [
    {
      title: 'Account',
      items: [
        { icon: Shield, label: 'KYC Documents', desc: 'Aadhaar, License, PAN', color: '#ffc107', action: () => setShowKYC(true) },
  ...
  return (
    <div className="pb-28">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl border-b" style={{ background: 'rgba(18,18,18,0.85)', borderColor: 'rgba(255,193,7,0.1)' }}>
        <div className="max-w-lg mx-auto px-4 py-4">
          <h1 className="text-lg font-black text-white flex items-center gap-2">
            <User size={20} className="text-[#ffc107]" /> Profile
          </h1>
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto px-4 pt-5 space-y-5">
        ...
        {/* Profile Card */}
        <div className="rounded-3xl p-5 border" style={{ background: 'linear-gradient(135deg, rgba(255,193,7,0.06), rgba(139,92,246,0.04))', borderColor: 'rgba(255,193,7,0.15)' }}>
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-black text-white">{user?.displayName || 'Rider'}</h2>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-[11px] text-gray-400 flex items-center gap-1"><Phone size={10} /> {user?.phone || rider?.phone || '9876543210'}</span>
              </div>
              <div className="flex items-center gap-2 mt-1">
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
            ...
            <div className="text-center">
              <p className="text-lg font-black text-[#ffc107]">4.8</p>
              <p className="text-[9px] text-gray-500">Rating</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-black text-emerald-400">97%</p>
              <p className="text-[9px] text-gray-500">Acceptance</p>
            </div>
          </div>
        </div>
        ...
        {menuSections.map(section => (
          <div key={section.title}>
            <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-2 px-1">{section.title}</h3>
        ...
        <p className="text-center text-[10px] text-gray-600 pb-4">Namma Ooru Express v2.0 • Rider App</p>
      </div>
      ...
      {showKYC && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full max-w-md rounded-t-3xl sm:rounded-3xl p-5 border max-h-[80vh] overflow-auto"
            style={{ background: '#1a1a1a', borderColor: 'rgba(255,193,7,0.15)' }}>
            <div className="w-10 h-1 bg-gray-700 rounded-full mx-auto mb-4" />
            <h2 className="text-lg font-black text-white mb-4 flex items-center gap-2">
              <Shield size={18} className="text-[#ffc107]" /> KYC Documents
            </h2>
                ...
                <div key={item.label} className="flex items-center gap-3 p-3.5 rounded-xl border"
                  ...
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                    ...
                    {item.status === 'verified' ? <CheckCircle2 size={16} className="text-emerald-400" /> :
                     item.status === 'not_required' ? <Clock size={16} className="text-gray-400" /> :
                  ...
                  <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${
                    ...
                    item.status === 'not_required' ? 'bg-gray-500/15 text-gray-400' :
